...
for idx in range(0, num_rows, 2):
    if highest_pop_country == None or cell(idx, "pop") > high_pop:
...
for idx in range(0, num_rows, 2):
    if highest_pop_inc_country == None or cell(idx+1, "pop") - cell(idx, "pop") > diff_pop:
        diff_pop = cell(idx+1, "pop") - cell(idx, "pop")
...
difference = 0
highest_nat_alb_inc_idx = None
for idx in range(0, num_rows, 2):
    if highest_nat_alb_inc_country == None or cell(idx+1, "national_alb") - cell(idx, "national_alb") > difference:
        difference = cell(idx+1, "national_alb") - cell(idx, "national_alb")
        highest_nat_alb_inc_country = cell(idx, "country_name")
        highest_nat_alb_inc_idx = idx
...
highest_alb_inc_income_level = cell(highest_nat_alb_inc_idx, "income_level")
...
all_urban_pop_percent_2015 = 0
for idx in range(0, num_rows, 2):
    all_urban_pop_percent_2015 += cell(idx, "pop") * (cell(idx, "urban_percent") / 100)
urban_pop_percent_2015 = round(all_urban_pop_percent_2015/ total_pop_2015 * 100)
...
levels_income = {}
for idx in range(0, num_rows, 2):
    bin_key = cell(idx, "income_level")
    if bin_key not in levels_income:
        levels_income[bin_key] = 0
        levels_income[bin_key] += cell(idx, "pop")
least_pop_income_group = None
for idx in levels_income:
    if least_pop_income_group == None or levels_income[idx] > levels_income[least_pop_income_group]:
        least_pop_income_group = idx
...
for idx in range(0, num_rows, 2):
    if cell(idx, "region") == "North America" and cell(idx, "national_alb") == max_nat_alb:
...
dict_country = get_col_dict('country_name', 2015)
dict_region = get_col_dict("region", 2015)
# call get_col_dict for other columns
dict_income = get_col_dict("income_level", 2015)
dict_year = get_col_dict("year", 2015)
dict_pop = get_col_dict("pop", 2015)
dict_urban = get_col_dict("urban_percent", 2015)
dict_national = get_col_dict("national_alb", 2015)
dict_urban_alb = get_col_dict("urban_alb", 2015)
...
for idx in range(0, num_rows, 2):
    ...
    dict_2015[country_code]["country_name"] = dict_country[country_code]
    dict_2015[country_code]["region"] = dict_region[country_code]
    dict_2015[country_code]["income_level"] = dict_income[country_code]
    dict_2015[country_code]["year"] = dict_year[country_code]
    dict_2015[country_code]["pop"] = dict_pop[country_code]
    dict_2015[country_code]["urban_percent"] = dict_urban[country_code]
    dict_2015[country_code]["national_alb"] = dict_national[country_code]
    dict_2015[country_code]["urban_alb"] = dict_urban_alb[country_code]
...
dict_country = get_col_dict("country_name", 2020)
dict_region = get_col_dict("region", 2020)
dict_income = get_col_dict("income_level", 2020)
dict_year = get_col_dict("year", 2020)
dict_pop = get_col_dict("pop", 2020)
dict_urban = get_col_dict("urban_percent", 2020)
dict_national = get_col_dict("national_alb", 2020)
dict_alb_dict = get_col_dict("urban_alb", 2020)

for idx in range(1, num_rows, 2):
    country_code = cell(idx, "country_code")
    dict_2020[country_code] = {}
    dict_2020[country_code]["country_name"] = dict_country[country_code]
    dict_2020[country_code]["region"] = dict_region[country_code]
    dict_2020[country_code]["income_level"] = dict_income[country_code]
    dict_2020[country_code]["year"] = dict_year[country_code]
    dict_2020[country_code]["pop"] = dict_pop[country_code]
    dict_2020[country_code]["urban_percent"] = dict_urban[country_code]
    dict_2020[country_code]["national_alb"] = dict_national[country_code]
    dict_2020[country_code]["urban_alb"] = dict_alb_dict[country_code]
...
income_level_pops = {}
for entry in dict_2020:
    if dict_2020[entry]["income_level"] not in income_level_pops:
        income_level_pops[dict_2020[entry]["income_level"]] = 0
    income_level_pops[dict_2020[entry]["income_level"]] += dict_2020[entry]["pop"]
income_level_pops
...
for entry in dict_2020:
    if dict_2020[entry]["income_level"] not in income_level_alb_pops:
        income_level_alb_pops[dict_2020[entry]["income_level"]] = 0
    income_level_alb_pops[dict_2020[entry]["income_level"]] += round(dict_2020[entry]["pop"] * dict_2020[entry]["national_alb"] / 100)
income_level_alb_pops
...
for entry in income_level_pops:
    income_level_alb_percent[entry] = round(income_level_alb_pops[entry]/income_level_pops[entry] * 100)
income_level_alb_percent
...
alb_rural_2015 = {}
alb_rural_2020 = {}
for entry in dict_2015:
    if dict_2015[entry]["urban_percent"] != 100:
        alb_rural_2015[entry] = round((dict_2015[entry]["national_alb"] - (dict_2015[entry]["urban_alb"] * dict_2015[entry]["urban_percent"]/100))/(1-(dict_2015[entry]["urban_percent"]/100)))
    else: alb_rural_2015[entry] = dict_2015[entry]["urban_alb"]
for entry in dict_2020:
    if dict_2020[entry]["urban_percent"] != 100:
        alb_rural_2020[entry] = round((dict_2020[entry]["national_alb"] - (dict_2020[entry]["urban_alb"] * dict_2020[entry]["urban_percent"]/100))/(1-(dict_2020[entry]["urban_percent"]/100)))
    else: alb_rural_2020[entry] = dict_2020[entry]["urban_alb"]
for entry in dict_2015:
    dict_2015[entry]["rural_alb"] = alb_rural_2015[entry]
    dict_2020[entry]["rural_alb"] = alb_rural_2020[entry]
...
for country in dict_2015:
    rural_non_alb = 100 - dict_2015[country]["rural_alb"]
    if 0 <= rural_non_alb < 10:
        rural_non_alb_bin_2015_dict[0].append(dict_2015[country]["country_name"])
    if 10 <= rural_non_alb < 20:
        rural_non_alb_bin_2015_dict[10].append(dict_2015[country]["country_name"])
    if 20 <= rural_non_alb < 30:
        rural_non_alb_bin_2015_dict[20].append(dict_2015[country]["country_name"])
    if 30 <= rural_non_alb < 40:
        rural_non_alb_bin_2015_dict[30].append(dict_2015[country]["country_name"])
    if 40 <= rural_non_alb < 50:
        rural_non_alb_bin_2015_dict[40].append(dict_2015[country]["country_name"])
    if 50 <= rural_non_alb < 60: 
        rural_non_alb_bin_2015_dict[50].append(dict_2015[country]["country_name"])
    if 60 <= rural_non_alb < 70:
        rural_non_alb_bin_2015_dict[60].append(dict_2015[country]["country_name"])
    if 70 <= rural_non_alb < 80:
        rural_non_alb_bin_2015_dict[70].append(dict_2015[country]["country_name"])
    if 80 <= rural_non_alb < 90:
        rural_non_alb_bin_2015_dict[80].append(dict_2015[country]["country_name"])
    if 90 <= rural_non_alb < 100:
        rural_non_alb_bin_2015_dict[90].append(dict_2015[country]["country_name"])
    if rural_non_alb == 100:
        rural_non_alb_bin_2015_dict[100].append(dict_2015[country]["country_name"])
rural_non_alb_bin_2015_dict
...
for country in dict_2020:
    rural_non_alb = 100 - dict_2020[country]["rural_alb"]
    if 0 <= rural_non_alb < 10:
        rural_non_alb_bin_2020_dict[0].append(dict_2020[country]["country_name"])
    if 10 <= rural_non_alb < 20:
        rural_non_alb_bin_2020_dict[10].append(dict_2020[country]["country_name"])
    if 20 <= rural_non_alb < 30:
        rural_non_alb_bin_2020_dict[20].append(dict_2020[country]["country_name"])
    if 30 <= rural_non_alb < 40:
        rural_non_alb_bin_2020_dict[30].append(dict_2020[country]["country_name"])
    if 40 <= rural_non_alb < 50:
        rural_non_alb_bin_2020_dict[40].append(dict_2020[country]["country_name"])
    if 50 <= rural_non_alb < 60: 
        rural_non_alb_bin_2020_dict[50].append(dict_2020[country]["country_name"])
    if 60 <= rural_non_alb < 70:
        rural_non_alb_bin_2020_dict[60].append(dict_2020[country]["country_name"])
    if 70 <= rural_non_alb < 80:
        rural_non_alb_bin_2020_dict[70].append(dict_2020[country]["country_name"])
    if 80 <= rural_non_alb < 90:
        rural_non_alb_bin_2020_dict[80].append(dict_2020[country]["country_name"])
    if 90 <= rural_non_alb < 100:
        rural_non_alb_bin_2020_dict[90].append(dict_2020[country]["country_name"])
    if rural_non_alb == 100:
        rural_non_alb_bin_2020_dict[100].append(dict_2020[country]["country_name"])
rural_non_alb_bin_2020_dict
...
for key in rural_non_alb_bin_2015_dict:
    if 20 <= key <= 29:
        bin_20_countries = rural_non_alb_bin_2015_dict[key]
...
for idx in range(0, 101, 10):
    if len(rural_non_alb_bin_2020_dict[idx]) == 0:
        last_non_empty_bin_2020 = rural_non_alb_bin_2020_dict[idx - 10]
        break
...
countries_regressed = []
for entry in dict_2015:
    country_name = dict_2015[entry]["country_name"]
    for country_bin in rural_non_alb_bin_2015_dict:
        if country_name in rural_non_alb_bin_2015_dict[country_bin]:
            bin_2015 = country_bin
    for country_bin in rural_non_alb_bin_2020_dict:
        if country_name in rural_non_alb_bin_2020_dict[country_bin]:
            bin_2020 = country_bin
    if bin_2020 > bin_2015:
